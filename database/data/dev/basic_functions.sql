...
-- Make per tournament score test
CREATE OR REPLACE FUNCTION make_per_tournament_score_test() RETURNS int LANGUAGE plpgsql AS $$
DECLARE
    tourn_id int := 0;
    tourn_name varchar := 'per_tournament_score_test';
    score_cat int := 0;
    score_id int := 0;
    ent_id int := 0;
BEGIN
